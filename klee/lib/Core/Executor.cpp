...
Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
    ...
    : Interpreter(opts), interpreterHandler(ih), searcher(0),
      ...
      /* [SGS]: Initialize sgs flag */ sgsUsingFlag(false),
      /* [SGS]: Initialize */ subpathCounts(std::vector<subpathCount_ty>(4)),
...
void Executor::terminateStateOnError(ExecutionState &state,
  ...
  if (EmitAllErrors ||
    ...
    auto stepName = getStepFilename("steps", interpreterHandler->getCurrentTestIndex() + 1);
...
void Executor::runFunctionAsMain(Function *f, int argc, char **argv,
  ...
  /// [Empc]: [FOCUS] the first execution state
  ExecutionState *state =
