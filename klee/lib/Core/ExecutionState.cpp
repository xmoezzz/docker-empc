...
ExecutionState::ExecutionState(const ExecutionState &state)
    : /* [Empc]: Copy Empc state utility */ mpcStateStepType(
          state.mpcStateStepType),
      pc(state.pc), prevPC(state.prevPC), stack(state.stack),
      incomingBBIndex(state.incomingBBIndex), depth(state.depth),
      addressSpace(state.addressSpace), stackAllocator(state.stackAllocator),
      heapAllocator(state.heapAllocator), constraints(state.constraints),
      ...
      /* [SGS]: */ takenBranches(state.takenBranches),
      instsSinceCovNew(state.instsSinceCovNew),
      unwindingInformation(state.unwindingInformation
                               ? state.unwindingInformation->clone()
                               : nullptr),
      coveredNew(state.coveredNew), forkDisabled(state.forkDisabled),
      base_addrs(state.base_addrs), base_mos(state.base_mos) {
  for (const auto &cur_mergehandler : openMergeStack)
...
void ExecutionState::pushFrame(KInstIterator caller, KFunction *kf) {
  ...
  /// [Empc]: push stack frame into the raw call stack
...
void ExecutionState::popFrame() {
  ...
  // klee::KInstIterator caller = sf.caller;

  /// [Empc]: pop stack frame from the raw call stack only when it is not
  /// destroying this `ExecutionState`
